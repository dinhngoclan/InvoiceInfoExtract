...
# import PIL
# print('PIL',PIL.__version__) #PIL 7.1.2
...
class TablePredict:
    ...
    def predict(self, image: Image) -> List[pd.DataFrame]:
        ...
        #cv2.cv2_imshow(self.inp_img)  ld
        ...
        #     cv2_imshow(abc*255)
        #cv2.cv2_imshow(self.inp_img*cv2.merge((table_mask, table_mask, table_mask))) ld
        #cv2.cv2_imshow(self.inp_img*cv2.merge((column_mask, column_mask, column_mask))) ld
        tables = []
        for table in segmented_tables:
...
def predict(image_path: str, model_weights: str) -> List[pd.DataFrame]:
    ...
    pred = TablePredict(model_weights, transforms)

    image = Image.open(image_path)
    tables = pred.predict(image)
    tableContainerSealNo = tables[-1]
    ...
    print(tableContainerSealNo)
    
    
#predict(r'.\InvoicesDirectory\TCDKO-2204-08 120422 (Kumho-2309)-01 NYLON-BUSAN-XKD.XLSxPACKING LIST.pdf.png', r'.\savedmodel\best_model.ckpt')
